...
let MAX_TIME_OF_TWEET: number = 1 * 60 * 1000; // 1 minute
...
const scriptRuntimeInput = usePrompt(
  "How long do you want the script to run? Enter in minutes(Default is 1 min): "
);
...
const timeInput = usePrompt(
  "How recent tweets do you want to scrape? Enter time in minutes(Default is 1 min): "
);
...
async function entrypoint() {
  while (Date.now() - startTime < SCRIPT_RUNTIME) {
    ...
    console.log("\n🕐30 secs Cooldown before next iteration...");
